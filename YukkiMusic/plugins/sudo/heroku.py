...
async def update_(client, message, _):
    ...
    if await is_heroku():
        try:
            ...
            for x in served_chats:
                try:
                    await app.send_message(
                        x,
                        f"**{config.MUSIC_BOT_NAME} baru saja me-restart (update) sistem, Maaf atas kendala ini.\n\nMulai bermain setelah 10-15 detik lagi** ✅",
                    )
            ...
            await response.edit(
                f"{nrs.text}\n\n**Terjadi masalah saat memulai ulang bot!  Silakan coba lagi nanti atau periksa LOG dan mulai ulang**"
            )
