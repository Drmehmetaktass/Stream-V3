...
async def update_(client, message, _):
    ...
    if await is_heroku():
        try:
            ...
            for x in served_chats:
                try:
                    await app.send_message(
                        x,
                        f"{config.MUSIC_BOT_NAME} baru saja me-restart(update) sistemnya sendiri, Maaf untuk untuk kendalanya.\n\nMulai bermain setelah 10-15 detik lagi.",
                    )
                    ...
                    pass
            await response.edit(
                f"{nrs.text}\n\nBot berhasil diperbarui di Heroku!  Sekarang, tunggu 2 - 3 menit sampai bot selesai merestart! ✅"
            )
            os.system(
            ...
            await response.edit(
                f"{nrs.text}\n\nTerjadi masalah saat memulai ulang bot!  Silakan coba lagi nanti atau laporkan ke owner jika terus menerus bermasalah"
            )
        ...
        for x in served_chats:
            try:
                ...
                pass
        await response.edit(
            f"{nrs.text}\n\nBot berhasil diperbarui!  Sekarang, tunggu 1 - 2 menit hingga bot selesai di reboot! ✅"
        )
